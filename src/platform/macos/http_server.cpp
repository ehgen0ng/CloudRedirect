...
static bool ValidateAndStripAuthToken(std::string& path) {
    const size_t query = path.find('?');
    if (query == std::string::npos || g_authToken.empty()) return false;
    const std::string expected = "cr_token=" + g_authToken;
    if (path.compare(query + 1, std::string::npos, expected) != 0) return false;
    path.resize(query);
    return true;
}
...
static void HandleConnection(int clientFd) {
    ...
    if (!ValidateAndStripAuthToken(path)) {
        LOG("[HttpServer] BLOCKED request with an invalid macOS auth token");
...
bool Start(const std::string& blobRoot, uint32_t accountId) {
    ...
    g_serverThread = std::thread([]() {
        while (g_running) {
            ...
            // The per-process URL token is validated in HandleConnection.
...
std::string AddAuthToken(const std::string& path) {
    return path + "?cr_token=" + g_authToken;
}
